...
@router.get(
    "/todos/", response_model=List[Todo], status_code=status.HTTP_200_OK, tags=["TODO"]
)
...
@router.get(
    "/todo/{id}", response_model=Todo, status_code=status.HTTP_200_OK, tags=["TODO"]
)
...
async def update_todo(
    id: int, todo_in: TodoIn, todos: CRUDTodo = Depends(get_database)
):
...
@router.delete(
    "/todo/{id}/delete", status_code=status.HTTP_204_NO_CONTENT, tags=["TODO"]
)
